            ...
            <div className="container mx-auto px-4">
                <div className="flex items-center justify-between h-[60px]">
                    ...
                    <div className="flex-1 mx-4 relative max-w-full md:max-w-[500px]">
                        <div className={`relative transition-all duration-300 rounded-full ${focused
                            ? "shadow-[0_0_0_3px_rgba(34,197,94,0.3)] border-green-400"
                            : "shadow-sm border-gray-200"
                            } border bg-white/70 backdrop-blur-md`}>
                            <SearchIcon className="absolute left-3 top-2.5 text-green-600" fontSize="small" />
                            <input
                                type="text"
                            ...
                            <div className="absolute top-13 left-0 w-full md:w-[500px] bg-white border border-gray-100 shadow-2xl rounded-b-2xl max-h-[70vh] overflow-y-auto z-50 animate-fade-down">
                                {loading ? (
                                    ...
                                    results.map((group, i) => (
                                        ...
                                        >
                                            <summary className="flex items-center justify-start px-4 py-2 bg-gray-50 hover:bg-green-50 text-green-700 font-semibold cursor-pointer select-none">
                                            ...
                                            <div className="p-3 grid grid-cols-1 gap-3">
                                                {group.results.map((item, idx) => (
                                                    <div
                                                        key={idx}
                                                        className="bg-white border border-gray-200 rounded-xl p-4 hover:border-green-300 hover:shadow-lg transition-all duration-200"
                                                    >
                                                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-4 gap-y-2 text-sm">
                                                            {Object.entries(item).map(([key, value]) => {
                                                                if (["_id", "createdOn", "__v"].includes(key)) return null;
                                                                return (
                                                                    <div key={key}>
