...
export default function Agent_Properties() {
    ...
    const fetchData = async () => {
        ...
        try {
            ...
            const soldProperties = allProperties.filter(property => property.owner?.clientType === "Agent");
    ...
    const extraColumns = [
        { accessorFn: row => `${row.owner?.name} (${row.owner?.phone})`, header: 'Agent', size: 80 },
        { accessorFn: row => row.employee ? `${row.employee.name} (${row.employee.phone})` : 'Pending...', header: 'Employee', size: 80 },
