...
export default function AddEdit({ open, onClose, data, refreshData }) {
    ...
    useEffect(() => {
        ...
        if (data) {
            setFormData((prev) => ({ ...prev, ...data, agent: loggedUser?.name || "", }));
        } else {
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            setErrors({
                ...backendErrors.includes?.('Source link already exists') && { sourceLink: 'Source link already exists.' }
            });
        } finally {
            setLoading(false);
        }
            ...
            <Modal open={open}>
                <Box sx={modalStyle} className="max-h-[95vh] overflow-y-auto">
                    ...
                    <Button
                        ...
                        disabled={loading}
                        className="!bg-[#4ea863] hover:!bg-green-700 !font-bold mt-4!"
                    >
                        {data ? "Update" : "Create"}
                    </Button>
